...
n_rand = 500;
n_trials = 7;
[ trials_r, trials_bb ] = meshgrid( linspace(0.1, 0.12, n_trials), linspace(0.01, 0.05, n_trials) );
trials = [ trials_r(:), trials_bb(:)];
...
Tmean      = zeros( length(trials), length( trajs ) );
Tvar       = zeros( length(trials), length( trajs ) );
BinsCount  = zeros( length(trials), length( trajs ) );
Tmax       = zeros( length(trials), length( trajs ) );
Ncollision = zeros( length(trials), length( trajs ) );
%%
%%% For each different experiment
for iTrial= 1:length( trials )
    disp('=========================================================');
    disp([num2str(iTrial),'/',num2str(length( trials ))])
    disp('=========================================================');    
for oo=1:length( trajs )
    ...
    [TrajPointsBin, TrajPoints, TrajBinOrder, TrajBinNElements, HocBinMean, HocBinSum, HocBinMax, HocBinMin ] = octree_calc( og_points, robot_traj, 50, og_occurences, n_rand, trials(iTrial,1), trials(iTrial,2),0 );
    ...
    if size( TexpectedMean, 1 ) > 0
        BinsCount  (iTrial, oo ) = BinCount;
        Tmean      (iTrial, oo ) = mean( TexpectedMean );
        Tvar       (iTrial, oo ) = std ( TexpectedMean );
        Tmax       (iTrial, oo ) = Tnom + Thuman;
        Ncollision (iTrial, oo ) = n_collision;
%         disp('----------------')
...
figure;
tmp = reshape( Tmean(:,1), [n_trials, n_trials] );
surf( trials_r, trials_bb, tmp );
xlabel( 'Rand Points Radius');ylabel('bb points radius');